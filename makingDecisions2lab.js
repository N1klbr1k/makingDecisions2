...
function yourGradeIs( aGrade ){
  let grades = ['A', 'B', 'C', 'D', 'F'];
  let gradeRange = [ [90, 100], [80, 89], [70, 79], [ 60, 69], [ 0, 60]];
   
  for(let i = 0; i < gradeRange.length; i++){ 
        if( aGrade > gradeRange[i][0] && aGrade < gradeRange[i][1] ){
...
if( myFavoriteNumbers.length < 7){
  console.log('There are not enough elements in this array');
   ...
   someNum = myFavoriteNumbers.splice[6, 0];
}
